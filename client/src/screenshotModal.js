...
// Interactive Screenshot Preview & Export Modal for TradingChart
...
export class ScreenshotModal {
  ...
  open(imageSrc) {
    ...
    modal.innerHTML = `
      <div class="modal-box" style="max-width: 640px; width: 94vw; background: #0c1017; border: 1px solid #1f293d; border-radius: 14px; box-shadow: 0 16px 48px rgba(0,0,0,0.8); overflow: hidden; display: flex; flex-direction: column;">
        <!-- Header -->
        <div style="padding: 14px 18px; background: #080b11; border-bottom: 1px solid #1c263c; display: flex; justify-content: space-between; align-items: center;">
          <div style="font-weight: 700; font-size: 14px; color: #fff; display: flex; align-items: center; gap: 8px;">
            ...
            <span>${isFa ? 'تصویر چارت تحلیلی با رزولوشن بالا' : 'High-Resolution Chart Snapshot'}</span>
          </div>
        ...
        <div style="padding: 16px; display: flex; flex-direction: column; gap: 14px;">
          ...
          <div style="width: 100%; max-height: 360px; background: #06090e; border: 1px solid var(--border-subtle); border-radius: 8px; overflow: hidden; display: flex; align-items: center; justify-content: center;">
            ${imageSrc ? `<img src="${imageSrc}" style="width: 100%; height: auto; object-fit: contain;" alt="Chart Snapshot" />` : `
              <div style="padding: 40px; text-align: center; color: var(--text-dim); font-size: 12px;">
                ...
                <div>${isFa ? 'تصویر چارت با موفقیت ذخیره شد.' : 'Chart snapshot ready for export.'}</div>
              </div>
            `}
          </div>
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
            <div style="font-size: 11px; color: var(--text-dim);">
              <span style="font-weight: 700; color: #fff;">${sym}</span> · ${tf} · ${new Date().toLocaleTimeString()}
            </div>
            <div style="display: flex; gap: 8px;">
              <button id="btn-dl-screenshot" class="btn-primary" style="padding: 6px 14px; font-size: 12px;">
                💾 ${isFa ? 'دانلود تصویر (PNG)' : 'Download PNG'}
              </button>
              <button id="btn-copy-screenshot-link" class="btn-secondary" style="padding: 6px 14px; font-size: 12px;">
